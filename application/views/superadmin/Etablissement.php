      ...
      <div class="modal-dialog modal-dialog-centered">
          <div class="modal-content">
              <div class="modal-header">
                  <h5 class="modal-title" id="staticBackdropLabel">Ajouter un etablissement</h5>
                ...
                <div class="modal-body">
                  <form action="<?php echo site_url('superadmin/Formulaire_controller/etablissement') ?>" method="post">
                      <div class="form-group">
                          <label for="exampleInputEmail1">Sigle</label>
                      ...
                      <?= form_error('nom') ?>
                        ...
                        <div class="form-group">
                          <label for="exampleInputEmail1">Numeros etablissement</label>
...
<!-- Content Wrapper. Contains page content -->
  ...
  <p class="card card-body vide"> GESTIONS DES ETABLISSEMENTS
    <button type="button" class="btn ajou" data-toggle="modal" data-target="#staticBackdrop"> 
                <h3 class="card-title"><b>
                  <i class="fas fa-edit"></i>
                  Ajouter un etablissement</b>
                </h3>
    </button></p>  
    <?php echo $data ?>
    <?= form_error('new_etab') ?>
    <?= form_error('new_name') ?>
    <?= form_error('new_contact') ?>
    <div class="card-body">
  <table id="example1" class="table table-bordered table-hover">
        <thead>
          <th class="mod">ID</th>
          <th class="concycle">SIGLE</th>
          <th>NOM</th>
          <th class="concycle">CONTACT</th>
          <th></th>
        </thead> 
        <?php $etablissement=$this->db->query('select * from etablissement');
            $i=1;
            foreach ($etablissement->result() as $etablissement) 
            {
          ?> 
        <tr>
          <td class="mod"><label><?php echo $i++ ?></label></td>
          <td class="concycle"><label><?php echo $etablissement->nom_etablissement ?></label></td>
          <td><label><?= $etablissement->sigle ?></label></td>
          <td class="concycle"><label><?php echo $etablissement->contact_etablissement ?></label></td>
             <td>
              <button class="collapsible orange" data-toggle="tooltip" title="modifier"><i class="fa fa-pen"></i></button>
              <div class="content">
                <form action="<?= site_url('superadmin/Affiche_controller/update_etablissement?id=').$etablissement->id_etablissement  ?>" method="post">
                    <div class="form-group row">
                      <label for="staticEmail" class="col-sm-2 col-form-label">sigle</label>
                      <div class="col-sm-4">
                        <input type="text" class="form-control" aria-describedby="emailHelp" name="new_etab" required="" value="<?= $etablissement->nom_etablissement ?>" maxlength="8"><?= form_error('new_etab') ?>
                      </div>
                    </div>
                    <div class="form-group row">
                      <label for="staticEmail" class="col-sm-2 col-form-label">Nom</label>
                      <div class="col-sm-4">
                        <input type="text" class="form-control" aria-describedby="emailHelp" name="new_name" required="" value="<?= $etablissement->sigle ?>"><?= form_error('new_name') ?>
                      </div>
                </div>
                <div class="form-group row">
                      <label for="staticEmail" class="col-sm-2 col-form-label">Contact</label>
                      <div class="col-sm-4">
                        <input type="number" class="form-control" aria-describedby="emailHelp" name="new_contact" required="" value="<?= $etablissement->contact_etablissement ?>"><?= form_error('new_contact') ?>
                      </div>
                      <button type="submit" class="btn btn-success btne">
                         modifier
                       </button>
                </div>
                </form>
