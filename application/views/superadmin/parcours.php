      ...
      <div class="modal-dialog modal-dialog-centered">
          <div class="modal-content">
              <div class="modal-header">
                  <h5 class="modal-title" id="staticBackdropLabel">Ajouter un parcours</h5>
                  <button type="button" class="close false" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                  </button>
                </div>
                <div class="modal-body">
                  <form autocomplete="off" action="<?= site_url('superadmin/parcours_controller/recuparcours') ?>" method="post">
                      <div class="form-group autocomplete">
                          <label for="exampleInputEmail1">Etablissement</label>
                          <input type="text" class="form-control" id="myInput" aria-describedby="emailHelp" required="" name="etablissement" value="<?= set_value('etablissement') ?>">
                          <?= form_error('etablissement') ?>
                        </div>
                       
                      <div class="form-group">
                        <?php
                          $cycle=$this->db->query('select * from cycle');
                        ?>
                          <label for="exampleInputEmail1">Cycle</label>
                          <select class="form-control" name="cycle">
                            <?php
                              foreach ($cycle->result() as $cycle) 
                              {
                            ?>
                            <option value="<?php echo $cycle->nom_cycle ?>">
                                <?php echo $cycle->nom_cycle ?>    
                            </option>
                            <?php
                              }
                             ?>
                      ...
                      <div class="form-group autocomplet">
                          ...
                          <input type="text" class="form-control" id="myInpu" aria-describedby="emailHelp" required="" name="filiere" value="<?= set_value('filiere') ?>">
                          <?= form_error('filiere') ?>
                        </div>

                     <div class="form-group">
                        <?php
                          $niveau=$this->db->query('select * from niveau');
                        ?>
                          <label for="exampleInputEmail1">Niveau</label>
                          <select class="form-control" name="niveau">
                            <?php
                              foreach ($niveau->result() as $niveau) 
                              {
                            ?>
                              <option value="<?php echo $niveau->nom_niveau ?>">
...
<!-- Content Wrapper. Contains page content -->
  ...
  <?php echo $data; ?>
        <?= form_error('etablissement') ?>
        <?= form_error('filiere') ?>
        <?= form_error('new_etablissement') ?>
        <?= form_error('new_filiere') ?>
            <table id="example1" class="table table-bordered table-hover">
              <thead>
               ...
               <?php
                  ...
                  ?>   
                <tr>
                  <td><label><?php echo $rows['id_parcours']; ?></label></td>
                  <td><label><?php echo $rows['sigle']; ?></label></td>
                  <td><label><?php echo $rows['nom_cycle']; ?></label></td>
                  <td><label><?php echo $rows['nom_filiere']; ?></label></td>
                  <td><label><?php echo $rows['nom_niveau']; ?></label></td>
                  <td>
                     <button class="collapsible orange" data-toggle="tooltip" title="modifier"><i class="fa fa-pen"></i></button>
                      <div class="content">
                  <form autocomplete="off" action="<?= site_url('superadmin/parcours_controller/update_parcours?id=').$rows['id_parcours']  ?>" method="post">
                          <div class="form-row">
                              <div class="form-group col-sm-4 autocomplete">
                                  <label for="exampleInputEmail1">Etablissement</label>
                                  <input type="text" class="form-control" id="myInput" aria-describedby="emailHelp" name="new_etablissement" value="<?php echo $rows['nom_etablissement']; ?>">
                                  <?= form_error('new_etablissement') ?>
                                </div>
                              ...
                              <div class="form-group col-sm-4">
                                ...
                                ?>
                                  <label for="exampleInputEmail1">Cycle</label>
                                  <select class="form-control" name="new_cycle">
                                    <?php
                                      foreach ($cycle->result() as $cycle) 
                                      {
                                    ?>
                            ...
                            <div class="form-row">
                              <div class="form-group col-sm-4 autocomplet">
                                  <label for="exampleInputEmail1">Filiere</label>
                                  <input type="text" class="form-control" id="myInpu" aria-describedby="emailHelp" name="new_filiere" value="<?php echo $rows['nom_filiere']; ?>">
                                  <?= form_error('new_filiere') ?>
                                </div>
                             ...
                             <div class="form-group col-sm-4">
                                ...
                                ?>
                                  <label for="exampleInputEmail1">Niveau</label>
                                  <select class="form-control" name="new_niveau">
                                    <?php
                                      foreach ($niveau->result() as $niveau) 
                                      {
                                    ?>
                  ...
                  <td>
                      <div class="icon-bar">
                         <button onclick="supp(<?php echo $rows['id_parcours']; ?>)" class="orange" data-toggle="tooltip" title="supprimer" ><i class="fa fa-trash">
                        </i></button>
                      </div>
                  </td>
                </tr>
 ...
 function supp(id){
    ...
    Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url : "<?php echo site_url('superadmin/Parcours_controller/delete_parcours')?>",
                        type: "POST",
                      data: {id:id},
...
function myFunctio() {
  ...
  document.getElementById("myBar").style.width = scrolled + "%";
        ...
        <?php
$filiere=$this->db->query('select * from filiere');
?>
    <script>
function autocomplet(inp, arr) 
{
  ...
  inp.addEventListener("input", function(e) 
  {
      ...
      a.setAttribute("id", this.id + "autocomplet-list");
      a.setAttribute("class", "autocomplet-items");
  ...
  {
      var x = document.getElementById(this.id + "autocomplet-list");
  ...
  function addActive(x) {
    ...
    x[currentFocus].classList.add("autocomplet-active");
  }
  function removeActive(x) {
    /*a function to remove the "active" class from all autocomplete items:*/
    for (var i = 0; i < x.length; i++) {
      x[i].classList.remove("autocomplet-active");
    }
  }
  function closeAllLists(elmnt) {
    /*close all autocomplete lists in the document,
    except the one passed as an argument:*/
    var x = document.getElementsByClassName("autocomplet-items");
    for (var i = 0; i < x.length; i++) {
...
var filiere = [ 
                  <?php 
                  foreach($filiere->result() as $filiere)
                  {
...
autocomplet(document.getElementById("myInpu"), filiere);
</script>
  <!-- /.content-wrapper -->
