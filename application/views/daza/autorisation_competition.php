...
<!-- Content Wrapper. Contains page content -->
  <div class="content-wrapper">
    ...
    <ul class="card card-body vide"> GESTIONS DES DEMANDES DE COMPETITIONS </ul>  
    <?php echo $data ?>
    <div class="card-body">
  <table id="example1" class="table table-bordered table-hover">
        <thead>
          ...
          <th></th>
            ...
            $recup_id_annee=$this->Daza_model->get_annee_encours();

            //RECUPERE LA COMPETITION (L'ID) D'UNE ANNEE NON CLOTURÉE
            $recup_id_competition=$this->Daza_model->get_competition_annee($recup_id_annee);
            foreach ($recup_id_competition as $recup_comp)
             {
                ...
                $recup_competition=$this->Daza_model->get_nom_annnee_etat_comp($id_comp);
          ...
          <tr>
            ...
            <td>
              <label>
                <?php echo $recup_id_etab['nom_etablissement'] ?>
              </label>
            </td>
            <td>
              <label>
                 <?php foreach ($recup_id_discipline as $recup_disc) 
                  {
                  echo $recup_disc['nom_discipline'].'<br>'; 
                  }
                  ?>
              </label>
            </td>
            <td>
              <label>
              <?php echo 'Du <label class="annee">'.$recup['date_debut'].'</label><br>Au <label class="annee">'.$recup['date_fin'].'</label>' ?>
              </label>
            </td>
            <td>
                        <input type="submit" class="btn btn-success btne" value="autoriser" name="valider" onclick="supp(<?php echo $id_comp ; ?>)">
                    </td>
                      <td>
                      <button class="btn btn-danger collapsible btnee" style="color: white;">decliner</button>
                   <div class="content"><br><br>
                    <form class="was-validated" method="post" action="<?php echo site_url('daza/Autorise_competition_controller/decline_competition?id=').$id_comp ?>">
...
<script>
 ...
 function supp(id){
    ...
    Swal.fire({
            title: 'Voulez-vous vraiment autoriser cette competition ?',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url : "<?php echo site_url('daza/Autorise_competition_controller/autorisation')?>",
                        type: "POST",
                      data: {id:id},
