...
<!-- Content Wrapper. Contains page content -->
  <div class="content-wrapper">
    ...
    <ul class="card card-body vide"> GESTIONS DES DEMANDES DE RENCONTRES </ul>  
    <?php echo $data ?>
    <div class="card-body">
  <table id="example1" class="table table-bordered table-hover">
        ...
        $recup_id_equip_ren=$this->Daza_model->get_id_equipe_rencontre();
            ...
            foreach ($recup_id_competition as $competition_discipline) 
            {
              ...
              $recup_id_etablissement=$this->Daza_model->get_id_etablissement($id_competition);
                  ...
                  <tr>
                    ...
                    <td>
                    <label>
                      <?php echo $competition_discipline['nom'].'<br>'.$competition_etablissement['sigle'] ?>
                    </label>
                  </td>
                  <td>
                    <label>
                    <?php echo 'Du <label class="orange">'.$competition_discipline['date_debut'].'</label><br> Au <label class="orange">'.$competition_discipline['date_fin'].'</label>' ?>
                    </label>
                    </td>
                    <td>
                      <label>
                    ...
                    <td>
                      <label>
                        <?php echo $equipe_rencontre['date_rencontre'].'<br> De '.$equipe_rencontre['debut_occupation'].' à '.$equipe_rencontre['fin_occupation'] ?>
                      </label>
                    </td>
                    <td>
                      <label>
                    ...
                    <td>
                        <input type="submit" class="btn btn-success btne" value="autoriser" name="autoriser" onclick="supp(<?php echo $equipe_rencontre['id_equip_rencontre'] ; ?>)">
                      </form>
                     </td>
                     <td>
                      <button class="btn btn-danger collapsible btnee" style="color: white;">decliner</button>
                   <div class="content"><br><br>
                    <form class="was-validated" method="post" action="<?php echo site_url('daza/Autorisation_controller/delete_rencontre?id=').$equipe_rencontre['id_equip_rencontre'] ?>">
                      <div class="input-group is-invalid">
                          <div class="custom-file">
                            <div class="mb-4">
                                  <textarea class="form-control is-invalid" id="validationTextarea" name="comment" placeholder="entrez un commentaire" required minlength="10" maxlength="100"></textarea>
                              </div>
                          <div class="input-group-append mb-4" style="height: 61px;">
                            <input type="submit" class="btn btn-danger btnee" value="ok" name="decliner">
                          </div>
                      </div>
                    </form>
                  </div>
 ...
 function supp(id){
    ...
    Swal.fire({
            title: 'Voulez-vous vraiment autoriser cette rencontre ?',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url : "<?php echo site_url('daza/Autorisation_controller/autorise')?>",
                        type: "POST",
                      data: {id:id},
                        success: function(data)
                        {
                          //if success reload ajax table
                            location.reload();
                            Swal.fire('<label class="success">autorisation effectuée avec succes</label>');
                        },
