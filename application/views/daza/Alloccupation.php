...
<head>
  ...
  <title><p><img src="<?php echo base_url('assets/img/image.jpg') ?>" class="rounded" alt="..." width="100px">Université de Ngaoundéré<center class="card card-body titre">CALENDRIER DES OCCUPATIONS ANNEE ACADEMIQUE <?php echo $academique ?></center></p> </title>
 ...
 <aside class="main-sidebar sidebar-dark-primary elevation-4">
    ...
    <div class="sidebar">
     ...
     <!-- Sidebar Menu -->
        ...
        <li class="nav-item">
              <a>
            <form class="form-inline" method="post" action="<?php echo site_url('daza/Alloccupation_controller/alloccupation') ?>">
...
<!-- Content Wrapper. Contains page content -->
  <div class="content-wrapper">
    ...
    <ul class="card card-body vide"> CALENDRIER DES OCCUPATIONS ANNEE ACADEMIQUE <?php echo $academique ?></ul>  
    <div class="card-body">
      ...
      foreach ($recup_id_competition as $recup_comp)
       {
          ...
          foreach ($recup_competition as $recup) 
          {
              ...
              foreach ($recup_id_etablissement as $recup_id_etab) 
              {
               ...
               foreach ($recup_id_discipline as $recup_disc) 
                  {
                   $id_discipline=$recup_disc['id_discipline']; 
                   //RECUPERE L'ID_PHASE DE LA TABLE PHASE_DISCIPLINE
                   $recup_id_phase_discipline=$this->Daza_model->get_id_discipline_phase($id_discipline);
                   foreach ($recup_id_phase_discipline as $phase_discipline) 
                   {
                     $id_phase_discipline=$phase_discipline['id_phase_discipline'];

                     //RECUPERE L'ID_RENCONTRE DE LA TABLE RENCONTRE
                   $recup_id_rencontre=$this->Daza_model->get_id_phase_rencontre($id_phase_discipline);
                   foreach ($recup_id_rencontre as $rencontre) 
                   {
                     $id_rencontre=$rencontre['id_rencontre'];

                  //RECUPERE L'EQUIPE_RENCONTRE DE LA TABLE RENCONTRE
                   $recup_equipe_rencontre=$this->Daza_model->get_equipe_rencontre($id_rencontre);
                   foreach ($recup_equipe_rencontre as $equipe_rencontre)
                   {
...
?>          
        <tr>
          ...
          <td><label><?php echo $recup_disc['nom_discipline']  ?></label></td>
          <td><label><?php echo $phase_discipline['nom_phase'] ?></label></td>
          <td><label><?php echo $rencontre['nom_stade'].' '.$rencontre['lieu_stade']  ?></label></td>
          <td><label><?php echo $rencontre['date_rencontre'].' '.$rencontre['debut_occupation'].' - '.$rencontre['fin_occupation']  ?></label></td>
          <td align="center"><label><?php echo '<label class="orange">'. $equipe_rencontre['nom'].' </label> <br> Vs<br><label class="orange"> '.$recup_equipe_adverse; ?></label></label></td>
        </tr>
   <?php
    }
    }
    ...
    <?php
     //RECUPERE L'ID AUTRE  DE LA TABLE ANNEE_AUTRE
      $recup_id_all_autre=$this->Daza_model->get_all_autre($id);
      foreach ($recup_id_all_autre as $annee_autre) 
      {
       ?>
       <tr>
           <td>
            <label><?php echo '<label class="orange">'.$annee_autre['date_autre'].'</label>' ?>
            </label>
          </td>
           <td><label><?php echo $annee_autre['nom_autre'] ?></label></td>
           <td><label class="erreur">RAS</label></td>
          <td><label class="erreur">RAS</label></td>
          <td><label class="erreur">RAS</label></td>
           <td><label><?php echo $annee_autre['nom_stade'].' '.$annee_autre['lieu_stade'] ?></label></td>
           <td>
            <label>
              <?php echo $annee_autre['date_autre'].'  '.$annee_autre['debut_occupation'].' - '.$annee_autre['fin_occupation']  ?>
              </label>
          </td>
          <td><label class="erreur">RAS</label></td>
       </tr>
